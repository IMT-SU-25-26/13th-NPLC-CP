...
export default async function DiscussionsPage(id: string) {
  // const discussions = await getAllDiscussions;
  // Asumsikan discussion adalah satu objek diskusi (BUKAN array)
  const discussion = {
    id: 1,
  ...
  const adminReplies = discussion.replies?.filter((r) => r.user.role === "ADMIN") || [];
  const userReplies = discussion.replies?.filter((r) => r.user.role !== "ADMIN") || [];
            ...
            <div className="flex flex-col gap-4 max-h-[60vh] overflow-y-auto">
              ...
              <div className="flex flex-col gap-1 mb-4">
                <div className="flex items-center gap-2">
                  <Link href="#" className="font-semibold text-white hover:underline text-sm md:text-base">
                    {discussion.user?.name || "Anonymous"}
                  </Link>
                  <span className="text-[#B9C0FF] text-xs">• {discussion.createdAt ? new Date(discussion.createdAt).toLocaleString() : ""}</span>
                </div>
                <p className="text-sm text-[#DFF7FA] mt-1">{discussion.content}</p>
              </div>

              {/* Jawaban admin di atas */}
              {adminReplies.map((reply, idx) => (
                <div key={reply.id || idx} className="flex flex-col gap-1 border-l-4 border-[#FCF551] pl-4 bg-[#23233a]/60 rounded">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-[#FCF551] text-sm md:text-base">Admin</span>
                    <Link href="#" className="font-semibold text-white hover:underline text-sm md:text-base">
                      {reply.user?.name || "Admin"}
                    </Link>
                    <span className="text-[#B9C0FF] text-xs">• {reply.createdAt ? new Date(reply.createdAt).toLocaleString() : ""}</span>
                  </div>
              ...
              <div className="flex items-start gap-3 mt-2">
                <div className="flex-1">
                  ...
                  <div className="flex justify-end mt-2">
                    <button className="bg-[#FCF551] text-black px-4 py-1 rounded-md font-semibold">Reply</button>
                  </div>
                </div>
              </div>
            </div>
